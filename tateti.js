  ...
  await tf.loadLayersModel(modelPath).then((model) => {
    ...
    const Jugada = (casilla) => {
      ...
      if (turno === jugadorX) {
        ...
        document.getElementById(`casilla-${casilla}`).textContent = "X";
      } else if (turno === jugadorO) {
        tablero[casilla] = jugadorO;
        document.getElementById(`casilla-${casilla}`).textContent = "O";
      } else {
    ...
    const jugarIA = async () => {
      ...
      result.data().then((data) => {
        ...
        if (jugadasDisponibles.length > 0) {
          Jugada(jugadasDisponibles[0].index);
        }
      });
    };
